<script>
  ...
  async function typewriterEffect(fullText) {
    ...
    for (let i = 0; i < fullText.length; i++) {
      ...
      await sleep(fullText[i] === "." || fullText[i] === "," ? 6 : 2);
      scrollChat();
    }
  ...
  $effect(() => {
    ...
    nickname = data.viewer?.nickname ?? nickname;
  ...
  function formatMessageText(value) {
    return renderParagraphs(linkifyText(
      escapeHtml(value)
...
<nav class="sidebar" class:open={navOpen}>
  ...
  {#if navOpen}
    <div class="nav-links">
      <a href={resolve("/app")} onclick={closeNav}>Home</a>
      <a href={resolve("/")} onclick={closeNav}>Landing</a>
    </div>
  {/if}
  ...
  <button class="profile-btn" onclick={openProfile} aria-label="Open profile">
    <div class="profile-avatar-small">
      {#if nickname}{nickname[0].toUpperCase()}{:else}P{/if}
    </div>
    {#if navOpen}
      <span class="profile-label">{nickname || "Profile"}</span>
    {/if}
  </button>
</nav>
  ...
  <div class="chatbox">
    ...
    <div class="chat-messages" bind:this={chatContainer}>
      ...
      {#each messages as msg}
        <div class="message {msg.role}"><div class="message-content">{@html formatMessageText(msg.text)}</div></div>
      {/each}
      {#if isStreaming}
        <div class="message ai">
          <div class="message-content">{@html formatMessageText(streamingText)}<span class="cursor">▋</span></div>
        </div>
      {/if}
...
{#if profileOpen}
  <div class="profile-modal" role="dialog" aria-modal="true">
    <div class="modal-header">
      ...
      <div>
        <h1 class="modal-title">{nickname || "Your Profile"}</h1>
    ...
    <div class="modal-body">
      ...
      <section class="section">
        <div class="section-row">
          ...
          <button class="add-btn" type="button" onclick={addSport}>+ Add Sport</button>
        </div>
        {#if sports.length === 0}
        ...
        {#each sports as sport, i}
          <div class="sport-row">
            ...
            <button class="remove-btn" type="button" onclick={() => removeSport(i)}>✕</button>
          </div>
        {/each}
      </section>
      ...
      <section class="section">
        ...
        <div class="custom-row">
          ...
          />
          <button class="add-btn" type="button" onclick={addCustomCondition}>Add</button>
        </div>
      </section>
      ...
      <section class="section">
        ...
        <div class="theme-row">
          {#each [["light", "Light"], ["dark", "Dark"], ["system", "System"]] as [t, label]}
            <button
              class="theme-btn"
              type="button"
      ...
      <div class="bottom-actions">
        <form
          ...
          use:enhance={({ formData }) => {
            saveProfile();
            formData.set("sports", JSON.stringify(sports));
            formData.set("problems", JSON.stringify(selectedConditions));
...
<style>
  ...
  :global(:root) {
    ...
    --chip-sel-t: #fff7f7;
    --sidebar-text: #f3e6e6;
    --sidebar-hover: #8a3434;
    --sidebar-border: rgba(255, 233, 233, 0.12);
  }
  :global([data-theme="dark"]) {
    ...
    --chip-sel-t: #fff5f5;
    --sidebar-text: #f2dddd;
    --sidebar-hover: #933939;
    --sidebar-border: rgba(255, 226, 226, 0.14);
  }
  :global(body) {
  ...
  .background-aura-left {
    ...
    left: 120px;
    top: 80px;
    background: radial-gradient(circle, rgba(110, 5, 5, 0.18), transparent 68%);
  }
  .background-aura-right {
    ...
    right: 260px;
    bottom: -80px;
    background: radial-gradient(circle, rgba(76, 76, 76, 0.16), transparent 70%);
  }
  .body-stage {
    position: relative;
    z-index: 1;
  ...
  .body-copy {
    ...
    left: 20px;
    z-index: 3;
    max-width: min(34rem, calc(100% - 420px));
    display: flex;
    flex-direction: column;
    gap: 12px;
    pointer-events: none;
  ...
  .chatbox {
    ...
    border-radius: 10px;
    box-shadow: 0 20px 48px rgba(27, 16, 16, 0.18);
    display: flex;
    flex-direction: column;
    overflow: hidden;
    z-index: 4;
  ...
  @media (max-width: 1200px) {
    .body-copy {
      max-width: min(28rem, calc(100% - 360px));
    }
